...
plt <- ggplot(tmpdata, aes(x = Group, y = Value, colour = Group)) +
     geom_point(position = position_jitterdodge(jitter.width = 0.8, dodge.width = 0.2),size=0.5) + # 添加抖动和分组
    #facet_grid(. ~ Genes) +
      stat_summary(aes(group = Group), fun = mean, geom = "point", 
               shape = 18, size = 5, color = "black", position = position_dodge(width = 0.75)) + # 添加平均值
  ...
  theme(axis.line = element_line(linewidth = 0.7), # 设置坐标轴线条的粗细
        axis.ticks = element_line(linewidth = 0.7),legend.position="none")+ # 设置坐标轴刻度线条的粗细
