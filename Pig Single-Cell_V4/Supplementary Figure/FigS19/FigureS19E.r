...
prop = read.csv("LD_sn_deconvolution-snRNAreal_Ctproportion_celltype11_20240717.csv")
colnames(prop)[1]='ID'
...
p1=ggplot(data=dff,aes(x=X1 ,y=Weight,size=2.5))+
   geom_point(color = '#040000',size=2.5)+
   ...
   theme(plot.title = element_text(vjust=0.5,hjust = 0.5,size=15,colour="black"),           
		 ...
		 geom_smooth(method = "lm", formula = y ~ x, color = "#224686",se=FALSE, size = .8)		 
